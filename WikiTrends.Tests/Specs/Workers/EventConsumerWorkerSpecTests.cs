...
[Trait("Category", "Spec")]
public sealed class EventConsumerWorkerSpecTests
{
    [Fact]
    public async Task ExecuteAsync_StartsConsumerOnTopicsOptionsClassifiedEdits_AndStopsOnCancellation()
    {
        var started = new TaskCompletionSource<string>(TaskCreationOptions.RunContinuationsAsynchronously);

        var consumer = new Mock<IKafkaConsumer<string, ClassifiedEditEvent>>(MockBehavior.Strict);
        consumer
        ...
        var topics = Options.Create(new TopicsOptions { ClassifiedEdits = "wiki.classified" });

        var worker = new EventConsumerWorker(consumer.Object, topics, NullLogger<EventConsumerWorker>.Instance);
